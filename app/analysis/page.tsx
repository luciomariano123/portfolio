...
// Argentine underlyings (country risk exposure), regardless of sector
const ARG_TICKERS = new Set(['PAMP', 'YPF', 'TGSU2', 'GGAL', 'BMA', 'BBAR', 'CEPU', 'VIST'])
...
export default function AnalysisPage() {
  const { consolidated, mounted } = usePositions()
  ...
  const posMetrics: PositionWithMetrics[] = useMemo(() => {
    ...
    for (const pos of consolidated) {
      ...
      if (!pd) continue
      const value = pos.quantity * pd.price
      ...
      rows.push({
        ...
        price: pd.price,
        dayPct: pd.changePercent,
        value,
        pct: 0, // filled below
        pnlUsd: value - costBasis,
  ...
  // Total P&L of CEDEARs
  const totalPnlUsd = posMetrics.reduce((s, p) => s + p.pnlUsd, 0)
  ...
  // Argentina exposure: by underlying ticker, not sector proxy
  const argValue = posMetrics.filter(p => ARG_TICKERS.has(p.ticker)).reduce((s, p) => s + p.value, 0)
  const argPct = cedearTotal > 0 ? (argValue / cedearTotal) * 100 : 0
  const argTickers = posMetrics.filter(p => ARG_TICKERS.has(p.ticker)).map(p => p.ticker)

  // Concentration: HHI over POSITION weights (standard measure)
  const posHhi = herfindahl(posMetrics.map(p => p.pct / 100))
  // Normalize: HHI of n equal positions = 1/n. Score 100 = perfectly spread across ≥10 positions.
  const diversScore = posMetrics.length > 0
    ? Math.max(0, Math.min(100, Math.round((1 - posHhi) / (1 - 1 / Math.max(posMetrics.length, 2)) * 100)))
    : 0
  ...
  const recs: Rec[] = useMemo(() => {
    ...
    // 2. Argentina risk (real, by underlying)
    if (argPct > 30)
      result.push({ tipo: 'alerta', titulo: 'Alta exposición Argentina', detalle: `${argPct.toFixed(0)}% en subyacentes argentinos (${argTickers.join(', ')}). Riesgo país concentrado.` })

    // 3. Single position overweight (top 2 only, to avoid spam)
    for (const pos of posMetrics.filter(p => p.pct > 20).slice(0, 2)) {
    ...
    // 4. Take profits (top 2 winners)
    for (const pos of posMetrics.filter(p => p.pnlPct > 60).slice(0, 2)) {
      result.push({ tipo: 'sugerencia', titulo: `Tomar ganancias en ${pos.ticker}`, detalle: `+${pos.pnlPct.toFixed(0)}% (${formatCurrency(pos.pnlUsd)}). Podrías realizarlas parcialmente.` })
    }

    // 5. Review thesis (top 2 losers)
    for (const pos of posMetrics.filter(p => p.pnlPct < -20).slice(0, 2)) {
      result.push({ tipo: 'alerta', titulo: `Revisar tesis en ${pos.ticker}`, detalle: `${pos.pnlPct.toFixed(0)}% bajo agua (${formatCurrency(pos.pnlUsd)}). Evaluar si el caso de inversión sigue válido.` })
    }

    // 6. Cash low
    if (cashPct < 5)
      result.push({ tipo: 'sugerencia', titulo: 'Baja liquidez', detalle: `${cashPct.toFixed(1)}% en cash. Difícil aprovechar caídas para comprar.` })
    ...
    if (numSectors >= 4 && techPct < 55 && top3pct < 50)
      result.push({ tipo: 'positivo', titulo: 'Buena diversificación', detalle: `${posMetrics.length} posiciones en ${numSectors} sectores con distribución razonable.` })
    ...
    return result
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mounted, posMetrics, techPct, argPct, cashPct, onPct, numSectors, ytdPct, top3pct])
    ...
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="flex items-end justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-6 text-right">
          <div>
            <p className="text-xs text-slate-500">P&L CEDEARs</p>
            <p className={`text-lg font-bold font-mono ${getPnlColor(totalPnlUsd)}`}>
              {totalPnlUsd >= 0 ? '+' : ''}{formatCurrency(totalPnlUsd)}
            </p>
          </div>
          <div>
            <p className="text-xs text-slate-500">YTD</p>
            <p className={`text-lg font-bold font-mono ${getPnlColor(ytdPct)}`}>{formatPercent(ytdPct)}</p>
          </div>
        </div>
      </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-800 rounded-xl p-5">
            ...
            <p className="text-xs text-slate-500 mt-2">{posMetrics.length} posiciones · {numSectors} sectores · HHI {posHhi.toFixed(2)}</p>
          </div>
          ...
          <div className="bg-slate-800 rounded-xl p-5">
            ...
            <p className="text-xs text-slate-500 mt-2">{argTickers.length > 0 ? argTickers.join(' · ') : 'Sin subyacentes argentinos'}</p>
          </div>
        </div>
      </section>
      ...
      {/* ── Posiciones ── */}
      <section>
        <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-3">Posiciones por Peso</h2>
          ...
          <table className="w-full text-sm min-w-[640px]">
            <thead>
              <tr className="border-b border-slate-700/50">
                ...
                <th className="text-right px-4 py-3 text-slate-400 font-medium">Precio</th>
                <th className="text-right px-4 py-3 text-slate-400 font-medium">Día</th>
                ...
                <th className="text-right px-4 py-3 text-slate-400 font-medium">P&L (USD)</th>
              ...
              {loading && posMetrics.length === 0 ? (
                ...
                posMetrics.map((pos) => (
                  <tr key={pos.ticker} className="hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className={`px-4 py-3 text-right font-mono text-xs ${getPnlColor(pos.dayPct)}`}>
                      {formatPercent(pos.dayPct)}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-slate-200">
                    ...
                    <td className={`px-4 py-3 text-right font-mono ${getPnlColor(pos.pnlUsd)}`}>
          ...
          {upcomingCoupons.length === 0 ? (
            ...
            <div className="space-y-3">
              {upcomingCoupons.map((c) => {
                ...
                const daysUntil = Math.ceil((new Date(c.date + 'T12:00:00').getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
                return (
